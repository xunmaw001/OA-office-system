    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(huiyi !=null){
            ...
            BeanUtils.copyProperties( huiyi , view );//把实体数据重构到view中
                ...
                //级联表
                HuiyishiEntity huiyishi = huiyishiService.selectById(huiyi.getHuiyishiId());
                if(huiyishi != null){
    ...
    */
    ...
    public R save(@RequestBody HuiyiEntity huiyi, HttpServletRequest request){
        ...
        Wrapper<HuiyiEntity> queryWrapper = new EntityWrapper<HuiyiEntity>()
            .eq("huiyishi_id", huiyi.getHuiyishiId())
...
//                            huiyiEntity.setHuiyishiId(Integer.valueOf(data.get(0)));   //会议室 要改的
//                            huiyiEntity.setHuiyiName(data.get(0));                    //会议主题 要改的
//                            huiyiEntity.setHuiyiTypes(Integer.valueOf(data.get(0)));   //会议类型 要改的
//                            huiyiEntity.setHuiyiTime(new Date(data.get(0)));          //会议时间 要改的
